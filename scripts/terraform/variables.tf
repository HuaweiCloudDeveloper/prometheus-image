...
variable "ecs_volume_size" {
  description = " Set the system disk size ( requires at least 50G or larger. The default value is 50G). "
  nullable    = false
  type    = number
  default = 50
}
...
variable "Version" {
  ...
  validation {
    condition     = contains(["Prometheus3.3.0-HCE2.0","Prometheus3.3.0-Ubuntu24.04"], var.Version)
